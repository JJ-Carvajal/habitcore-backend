...
async function cargarCategoriasUI() {
    const token = localStorage.getItem('token');
    const res = await fetch('https://habitcore-backend.onrender.com/api/habitos/categorias', {
...
async function cargarDashboard(usuarioId) {
    try {
        mostrarLoading(true);
        const token = localStorage.getItem('token');
        const res = await fetch(`https://habitcore-backend.onrender.com/api/habitos?usuarioId=${encodeURIComponent(usuarioId)}`, {
            headers: { 'Authorization': `Bearer ${token}` }
        });
...
async function actualizarEstadisticas(usuarioId) {
    try {
        const token = localStorage.getItem('token');
        const res = await fetch(`https://habitcore-backend.onrender.com/api/habitos/estadisticas?usuarioId=${encodeURIComponent(usuarioId)}`, {
            headers: { 'Authorization': `Bearer ${token}` }
        });
...
async function marcarCompletado(id) {
    try {
        ...
        const token = localStorage.getItem('token');
        const res = await fetch('https://habitcore-backend.onrender.com/api/habitos/registros', {
            method: 'POST',
...
async function eliminarHabito(id) {
    try {
        ...
        const token = localStorage.getItem('token');
        const usuario = JSON.parse(localStorage.getItem('usuario'));
        const res = await fetch(`https://habitcore-backend.onrender.com/api/habitos/${id}`, { method: 'DELETE', headers: { 'Authorization': `Bearer ${token}` } });
...
async function guardarHabito(e) {
    ...
    btn.disabled = true;
    try {
        const res = await fetch('https://habitcore-backend.onrender.com/api/habitos', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` },
...
async function guardarEdicionHabito(e) {
    ...
    btn.disabled = true;
    try {
        const res = await fetch(`https://habitcore-backend.onrender.com/api/habitos/${id}`, {
            method: 'PUT',
            headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` },
            body: JSON.stringify(data)
        });
...
async function cargarProgreso() {
    try {
        mostrarLoading(true);
        const token = localStorage.getItem('token');
        const res = await fetch('https://habitcore-backend.onrender.com/api/progreso', {
            headers: { 'Authorization': `Bearer ${token}` }
        });
